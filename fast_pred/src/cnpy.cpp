...
void cnpy::parse_npy_header(std::string &buffer, size_t &word_size,
  ...
  uint8_t major_version = *reinterpret_cast<const uint8_t *>(buffer.data() + 6);
  uint8_t minor_version = *reinterpret_cast<const uint8_t *>(buffer.data() + 7);

  (void)major_version;
  (void)minor_version;

  uint16_t header_len =
      (*reinterpret_cast<const uint8_t *>(buffer.data() + 8)) |
      ((*reinterpret_cast<const uint8_t *>(buffer.data() + 9)) << 8);
